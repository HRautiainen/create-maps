...
CRS provide a standardized way of describing locations to describe geographic data.
The CRS that is chosen depends on when the data was collected, the geographic extent of the data, 
the purpose of the data, etc.

In R, when data with different CRS are combined, it is important to transform them to a common CRS so they
align with one another. This is similar to making sure that units are the same.
...
More reading: https://www.nceas.ucsb.edu/sites/default/files/2020-04/OverviewCoordinateReferenceSystems.pdf
...
# swe_no_sweref <- sf::st_transform(swe_no, crs = CRS("+init=epsg:3006")) # Transform or convert coordinates of simple feature (sf)
...
# sf_use_s2(FALSE) # if errors occur 
lakes <- st_crop(lakes, sweden.c) # crop an sf object to a specific rectangle (here based on extent of "sweden.c"-shp)
...
# create xy points with labels
# data.frame with four different column one for each variable
...
ggplot() +
  ...
  annotation_north_arrow(location = "bl", 
                       ...
                       pad_y = unit(8.5, "cm"), # vertical align
...
## Exercise 1: Create a polygon and add to map 
```{r ignore, echo=FALSE}
...
# projection to use: here by using proj4string instead of ESPG code
crs_to_use <- "+proj=utm +zone=33 +ellps=GRS80 +units=m +no_defs" #proj4string 

# crs_to_use <- CRS("+init=epsg:3006") #ESPG 
