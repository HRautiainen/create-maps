...
# data to read
input_raster <- list.files(spatial_folder,
                          pattern = ".tif",
                          full.names = T,
                          recursive = T)
...
# # maps to ignore 
# ignore <- c("nmd2018bas_generaliserad_v1_0")

# maps to read
maps_files_read <- grep(pattern = paste(ignore, collapse = "|"), 
                        input_raster, value = T, invert = T)
...
reindeer_data <- readRDS("input_data/vector/animal_data/data_cleaned_PV.rds") %>%
  # dplyr::as_tibble()  %>%
  dplyr::select(Collar_ID, Object_ID, burst, id, year, x, y, Timestamp, Info)  %>%
  subset(Timestamp >= as.POSIXct('2021-01-28 00:00', tz = "GMT") &
           Timestamp <= as.POSIXct('2021-04-07 23:59', tz = "GMT")) %>%
  filter(Info == "Control1"|Info == "Fed1") %>%
    dplyr::rename(group = Info) %>%
   dplyr::mutate(
    group = case_when(
      group == "Control1"  ~ "Control", 
      group == "Fed1"  ~ "Fed"))
...
move <- SpatialPoints(reindeer_data[, c("x", "y")], 
                      proj4string = CRS("+proj=utm +zone=33 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))
move_fed <- SpatialPoints(fed_ids[, c("x", "y")], 
                      proj4string = CRS("+proj=utm +zone=33 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))

move_ctrl <- SpatialPoints(ctrls[, c("x", "y")], 
                      proj4string = CRS("+proj=utm +zone=33 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))
...
#   grep(pattern = "lakes", value = T) %>%
#   sf::st_read()
# lakes2 <- st_transform(lakes2, crs = crs(elevation))
# sf_use_s2(FALSE)
# lakes2 <- st_crop(lakes2, elevation)
# plot(lakes2)
...
## make animal positions df
move_fed <- move_fed%>%
  st_as_sf(coords = c("x", "y"),
           crs=elevation)
move_fed <- st_transform(move_fed, crs = crs(elevation))
move_fed2 <- st_crop(move_fed, elevation)

move_ctrl <- move_ctrl%>%
  st_as_sf(coords = c("x", "y"),
           crs=elevation)
move_ctrl <- st_transform(move_ctrl, crs = crs(elevation))
move_ctrl2 <- st_crop(move_ctrl, elevation)


move <- move%>%
  st_as_sf(coords = c("x", "y"),
           crs=elevation)
move <- st_transform(move, crs = crs(elevation))
move2 <- st_crop(move, elevation)
...
ggplot() +
  ...
  # add animal data 
  geom_sf(data = move_fed2, colour="red") + 
  geom_sf(data = move_ctrl2, colour="black") + 

  # doesn't work with lakes
  # geom_sf( 
  #   data = lakes,
...
# pg1_B <- ggplotGrob(m)
# pg2_B <- ggplotGrob(m)
# maxHeight = grid::unit.pmax(pg1_B$heights, pg2_B$heights)
# pg1_B$heights <- as.list(maxHeight)
# pg2_B$heights <- as.list(maxHeight)
# maxWidth = grid::unit.pmax(pg1_B$widths, pg2_B$widths)
# pg1_B$widths <- as.list(maxWidth)
# pg2_B$widths <- as.list(maxWidth)
# 
# pg1_C <- ggplotGrob(m)
# pg2_C <- ggplotGrob(m)
# maxHeight = grid::unit.pmax(pg1_C$heights, pg2_C$heights)
# pg1_C$heights <- as.list(maxHeight)
# pg2_C$heights <- as.list(maxHeight)
# 
# maxWidth = grid::unit.pmax(pg1_C$widths, pg2_C$widths)
# pg1_C$widths <- as.list(maxWidth)
# pg2_C$widths <- as.list(maxWidth)
